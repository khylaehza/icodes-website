...
const Form = () => {
	...
	const [emailExists, setEmailExists] = useState(curUser.Email? true : false);
    const [employeeProfile, setEmployeeProfile] = useState(curUser.Image? curUser.Image : null);
    const [employeeName, setEmployeeName] = useState(
		curUser.Email?
			`${curUser.FName.charAt(0).toUpperCase() + curUser.FName.slice(1)} 
	...
	const form = useFormik({
        ...
        onSubmit: (values,action) =>{
            ...
            if (emailExists) {
                const emp = employees.filter((employee)=> employee.Email === email);
                const img = emp.map((employee) => employee.Image);
                const fullName = emp.map((employee) => 
                `${employee.FName.charAt(0).toUpperCase() + employee.FName.slice(1)} 
