...
import { CusSelectTowerLoan, CusNumInput, CusSelectYearsToCal, CusNumInputLeftAdd, CusNumInputRightAdd } from "../../customs";
...
const LoanPaymentCalculator = () =>{
    ...
    const RemainTCP = loans.filter((item) => item.Tower === tower).map(itm => itm.RemainTCP)
    ...
    const form = useFormik({
        initialValues:{
            ...
            const result = (Loanable * interest) /months
            console.log(result)


            const Result = result.toLocaleString('en-US', {
                style: 'currency',
                currency: 'PHP' 
            });
    ...
    return(
        <>
            <Box 
                ...
                gap={3}
            >
                <Box  display={'flex'} flexDir={{xl: 'row', sm: 'column'}} w={tower ? '80%' : '30%'} gap={3}>
                    <CusSelectTowerLoan
                        label={'Tower'}
                        name='tower'
                        id='tower'
                ...
                <Box  display={'flex'} flexDir={{xl: 'row', sm: 'column'}} w={{xl: '70%', sm: '80%'}} gap={3}>
                        <CusNumInputRightAdd 
                            name= 'interest'
                            id={'interest'}
                            label={'Interest rate'}
            ...
            {displayresult.value? (<Text>Your estimated playment loan is {displayresult.value} for ({displayresult.months} months)  </Text>):('')}
        </>
    )
}
