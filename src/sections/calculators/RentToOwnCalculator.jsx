...
const RenToOwnCalculator = () => {
	...
	let ReservationFee = payterm
		.filter((item) => item.PaymentTermName === pyterm)
		.map((itm) => itm.ReservationFee);
	const form = useFormik({
		...
		onSubmit: (value, action) => {
			const numMonth = payterm
				.filter((item) => item.PaymentTermName === value.payterm)
				.map((itm) => itm.NoOfMonths);
			const monthlyP = payterm
				.filter((item) => item.PaymentTermName === value.payterm)
				.map((itm) => itm.MonthlyPercent);
			...
			const v = monthlyPercent * total_tcp;
			const result = v - resevationInt;

			const fResult = result / number_of_months;
			const formattedResult = fResult.toLocaleString('en-US', {
				style: 'currency',
				currency: 'PHP',
			});
		...
		<>
			...
			{displayresult.value ? (
				<Text
					...
					alignSelf={'center'}
				>
					Your estimated monthly payment for rent-to-own is{'  '}
